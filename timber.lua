...
local scriptRuntimeEnv = (type(getgenv) == "function" and getgenv()) or _G
local existingRuntimeGui = playerGui:FindFirstChild("TimberScriptByVaanGUI")
    or playerGui:FindFirstChild("AutoPerfectChopGUI")
if scriptRuntimeEnv.__TimberScriptByVaanRunning and existingRuntimeGui then
    warn("Timber Script already running. Skip duplicate loadstring.")
    return function()
        return nil
    end
end
scriptRuntimeEnv.__TimberScriptByVaanRunning = true
...
function releaseRuntimeLock()
    if scriptRuntimeEnv then
        scriptRuntimeEnv.__TimberScriptByVaanRunning = false
    end
end
...
farmForceTreeInfoToggleButton.Visible = false
